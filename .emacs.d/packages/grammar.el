;;;
;;; 編集版。add-hook post-command-hookの方法と、grammar-check-pre-sentence-pの条件を変更
;;;
...
(defun grammar-mode-on ()
  ...
  (add-hook 'post-command-hook (function grammar-post-command-hook) nil t)
...
(defun grammar-sentence-end-command-p ()
  (or (eq this-command 'self-insert-command)
      (eq this-command 'org-self-insert-command)
      (eq this-command 'orgtbl-self-insert-command)
      (eq this-command 'newline)
      (eq this-command 'TeX-newline)
      (eq this-command 'indent-for-tab-command)
      (eq this-command 'orgtbl-hijacker-command-102)))

(defun grammar-sentence-end-char-p ()
  (let ((current-char (char-before (point)))
        (before-char (char-before (1- (point)))))
    (or (and (eq current-char ? ) (not (eq before-char ? )))
        (and (eq current-char ?.) (not (eq before-char ?.)))
        (and (eq current-char 10) (not (eq before-char 10)))
        )))

(defun grammar-sentence-english-p ()
  (let (start end)
    (save-excursion
      (backward-sentence)
      (setq start (point))
      (forward-sentence)
      (setq end (point)))
    (eq (string-match "^[a-zA-Z-,.'\"/() \t\n]+$" (buffer-substring start end)) 0)))
...
(defun grammar-check-pre-sentence-p ()
  ...
  (let ((sentence-end (sentence-end)))
    (cond
     ((and (grammar-sentence-end-command-p)
           (grammar-sentence-end-char-p)
